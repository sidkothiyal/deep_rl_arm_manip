...
#define COLLISION_POINT_GRIPPER	"arm::gripper_base::gripper_collision"
#define COLLISION_POINT_GRIPPER_MIDDLE	"arm::gripper_middle::middle_collision"
#define COLLISION_POINT_GRIPPER_LEFT	"arm::gripper_left::left_gripper"
#define COLLISION_POINT_GRIPPER_RIGHT	"arm::gripper_right::right_gripper"

//Arm
#define COLLISION_ARM_JOINT1  "arm::joint1::collision"
#define COLLISION_ARM_LINK1  "arm::link1::collision"
#define COLLISION_ARM_JOINT2  "arm::joint2::collision"
#define COLLISION_ARM_LINK2  "arm::link2::collision2"
...
void ArmPlugin::onCollisionMsg(ConstContactsPtr &contacts)
{
	...
	for (unsigned int i = 0; i < contacts->contact_size(); ++i)
	{
		...
		*/
		...
		bool collisionCheckItem = (strcmp(contacts->contact(i).collision1().c_str(), COLLISION_ITEM) == 0);
          	
		if (collisionCheckItem)
		{
          	bool collisionCheckPoint = ( strcmp(contacts->contact(i).collision2().c_str(), COLLISION_POINT) == 0 ||
                                       strcmp(contacts->contact(i).collision2().c_str(), COLLISION_POINT_GRIPPER) == 0 ||
                                       strcmp(contacts->contact(i).collision2().c_str(), COLLISION_POINT_GRIPPER_MIDDLE) == 0 ||
                                       strcmp(contacts->contact(i).collision2().c_str(), COLLISION_POINT_GRIPPER_LEFT) == 0 ||
                                       strcmp(contacts->contact(i).collision2().c_str(), COLLISION_POINT_GRIPPER_RIGHT) == 0 ||
                                       strcmp(contacts->contact(i).collision2().c_str(), COLLISION_ARM_JOINT1) == 0 ||
                                       strcmp(contacts->contact(i).collision2().c_str(), COLLISION_ARM_LINK1) == 0 ||
                                       strcmp(contacts->contact(i).collision2().c_str(), COLLISION_ARM_JOINT2) == 0 ||
                                       strcmp(contacts->contact(i).collision2().c_str(), COLLISION_ARM_LINK2) == 0);
...
bool ArmPlugin::updateAgent()
{
	...
	*/
	...
	float velocity = vel[action/2] + (actionVelDelta * mul); // TODO - Set joint velocity based on whether action is even or odd.
	...
	*/
	...
	if (action % 2 == 0)
		...
		mul = -1.0f;
	float joint = ref[action/2] + (actionJointDelta * mul); // TODO - Set joint position based on whether action is even or odd.
...
void ArmPlugin::OnUpdate(const common::UpdateInfo& updateInfo)
{
	...
	if( hadNewState && !newReward )
	{
		...
		*/ 
		...
		if(!checkGroundContact)
		{
			...
			if( episodeFrames > 1 )
			{
				const float distDelta  = (lastGoalDistance - distGoal)/distGoal;
				float alpha = 0.2;
				// compute the smoothed moving average of the delta of the distance to the goal
              	avgGoalDelta  = (avgGoalDelta * alpha) + (distDelta * (1.0f - alpha));
				rewardHistory = avgGoalDelta * REWARD_INTER;
				//printf("distDelta %f, avgGoalDelta %f, rewardHistory %f \n", distDelta, avgGoalDelta, rewardHistory);
				newReward     = true;	
			}
	...
	if( newReward && agent != NULL )
	{
		...
		if( endEpisode )
		{
			...
			success[successEnd] = false;
             if( rewardHistory >= REWARD_WIN ){
				successScore++;
            	success[successEnd] = true;
            }
         	if( successStart == successEnd ){
                successEnd = (successEnd + 1)%100;
              	total ++;
            } else if ((successEnd + 1) % 100 == successStart){
            	successScore -= success[successStart];
              	successEnd = (successEnd + 1) % 100;
              	successStart = (successStart + 1) % 100;
              	total = 100;
            } else {
            	successEnd = (successEnd + 1) % 100;
              	total++;
			...
			totalRuns++;
			printf("Current Accuracy (in last 100 attempts); total attempts %d:  %0.4f (%03u of %03u)  (reward=%+0.2f %s)\n", totalRuns, (float(successScore)/float(total) *100), successScore, total, rewardHistory, (rewardHistory >= REWARD_WIN ? "WIN" : "LOSS"));
